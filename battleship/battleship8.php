...
$game_guesses_filename = "gamedata/game_guesses_$session.txt";
...
$other_game_guesses_filename = "gamedata/game_guesses_$other_session.txt";
...
if ($startover) {
    ...
    if ($other_game_state == "play" && $game_state == "play") {
        $other_game_board = unserialize(file_get_contents($other_game_board_filename));
        $other_game_guesses = unserialize(file_get_contents($other_game_guesses_filename));
    }
}
...
for ($row=0;$row<10;$row++) {
    for ($col=0;$col<10;$col++) {
        if ($game_board[$row][$col]) $total_placed = $total_placed + 1;
    }
}
...
file_put_contents($game_guesses_filename, serialize($game_guesses));
?>
...
<form id=myform action="battleship8.php" method="get">
...
<input type=button value="Sync Sessions" onclick="sync()">
</form>
...
for ($row=0;$row<10;$row++) {
    for ($col=0;$col<10;$col++) {
        $row_letters = "ABCDEFGHIJ";
        if ($game_board[$row][$col] || $other_game_guesses[$row][$col]) {
            $col_name = $col + 1;
            ...
            else if ($game_board[$row][$col] && !$other_game_guesses[$row][$col]) {
                $color = "gray"; // not guesses
            }
            else {
                $color = "yellow"; // miss
            }
        ...
        if ($game_guesses[$row][$col]) {
            $col_name = $col + 1;
            $row_name = $row_letters[$row];
            // add E to other
            $id = "E$row_name$col_name";
            
            if ($other_game_board[$row][$col]) {
                $color = "red"; // hit
            }
            else {
                $color = "gray"; // not 
        ...
        if ($other_game_board[$row][$col] && !$game_guesses[$row][$col]) {
            $win = false;
        }
    }
}
...
// the interval interferes with the ability to use the <input> box for session,
// so I extended it out
